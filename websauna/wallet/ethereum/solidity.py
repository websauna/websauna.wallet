# Courtesy of https://github.com/pipermerriam/populus/blob/master/populus/solidity.py
# Python3nized
...
is_solc_available = which(SOLC_BINARY)


def solc_version():
    version_string = subprocess.check_output(['solc', '--version']).decode("utf-8")
...
def solc(source=None, input_files=None, add_std=True,
    ...
    if source:
        stdoutdata, stderrdata = p.communicate(input=source.encode("utf-8"))
    else:
        stdoutdata, stderrdata = p.communicate()

    if p.returncode:
        raise CompileError('compilation failed: ' + stderrdata.decode("utf-8"))
